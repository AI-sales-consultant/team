...
def get_openai_client():
    ...
    if not api_key or not endpoint:
        print("警告: Azure OpenAI 凭据未配置，LLM 功能将不可用")
        return None
    ...
    try:
        ...
        print(f"警告: Azure OpenAI 客户端初始化失败: {e}")
        return None
...
async def get_llm_advice(request: LLMAdviceRequest):
    ...
    if not client:
        return {
            "advice": "LLM 服务未配置，请检查 Azure OpenAI 凭据设置。",
            "timestamp": datetime.utcnow().isoformat()
        }
    ...
    try:
        ...
        print(f"处理LLM建议时出错: {e}")
        return {
            "advice": f"处理请求时出错: {str(e)}",
            "timestamp": datetime.utcnow().isoformat()
        }
