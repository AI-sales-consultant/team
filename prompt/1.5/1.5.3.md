...
## Performance Metrics
- When generating answers, more attention should be given to performance metrics, combining the variables provided by the client to design a set of KPIs for measuring the company’s performance (such as growth rate, average deal value, customer acquisition cost (CAC), customer lifetime value (LTV), gross profit, revenue, etc.).
